...
def find_visible_tree(_input: List[List[int]]) -> Set[Point]:
    visible = set()
...
def compute_scenic_score(start: Point, _input: List[List[int]]) -> int:
    ...
    while queue:
        ...
        if not (0 <= x + dx <= max_x) or not (0 <= y + dy <= max_y):
            sol.append(steps)
        elif _input[x + dx][y + dy] >= height:
            sol.append(steps + 1)
        else:
            queue.append((steps + 1, dir, (x + dx, y + dy)))
...
def main(filename: str) -> Solution:
    ...
    visible_trees = find_visible_tree(_input)
